...
def on_agent_event_callback(event, uuid, name, event_data, my_data):
    try:
        ...
        assert isinstance(agent_object, clavier)
...
# inputs
def New_Input_input_callback(io_type, name, value_type, value, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, clavier)
        agent_object.New_InputI = value
...
# services
def Sendnotes_callback(sender_agent_name, sender_agent_uuid, service_name, tuple_args, token, my_data):
    try:
        agent_object = my_data
        assert isinstance(agent_object, clavier)
        agent_object.Sendnotes(sender_agent_name, sender_agent_uuid)
    except:
        print(traceback.format_exc())
...
def elementCreated_callback(sender_agent_name, sender_agent_uuid, service_name, tuple_args, token, my_data):
    try:
        ...
        firstElement = tuple_args[0]
        agent_object.registerElement(firstElement)
    except:
        print(traceback.format_exc())
...
if __name__ == "__main__":
    ...
    igs.definition_set_class("clavier")
    ...
    igs.input_create("new_input", igs.STRING_T, None)
    igs.observe_input("new_input", New_Input_input_callback, agent)

    igs.service_init("sendNotes", Sendnotes_callback, agent)
    igs.service_init("elementCreated", elementCreated_callback, agent)
    igs.service_arg_add("elementCreated", "ID", igs.INTEGER_T)
